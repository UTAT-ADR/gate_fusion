...
#ifndef ATTITUDE_CONTROLLER_ROTATION_HPP_
#define ATTITUDE_CONTROLLER_ROTATION_HPP_
 ...
 *
 * @param mat A skew-symmetric matrix
 ...
 *
 * @param angle_axis The angle-axis vector, whose norm is an angle in radians
 ...
 *
 * @param rotation_matrix rotation matrix, a 3-by-3 matrix that is orthonormal
 * with determinant 1
...
Eigen::Matrix<typename Derived::Scalar, 3, 3> AngleAxisToRotationMatrix(
  ...
  if (abs(theta_sq) < Scalar(1e-8)) {
    rotation_matrix += hat_phi + Scalar(0.5) * hat_phi_sq;
  } else {
...
Eigen::Quaternion<typename Derived::Scalar> EulerAnglesToQuaternion(
    const Eigen::MatrixBase<Derived>& euler_angles) {
  using Scalar = typename Derived::Scalar;
  using std::cos;
  using std::sin;

  const Scalar roll = euler_angles.x();
  const Scalar pitch = euler_angles.y();
  const Scalar yaw = euler_angles.z();
  const Scalar cr = cos(roll * 0.5);
  const Scalar sr = sin(roll * 0.5);
  const Scalar cp = cos(pitch * 0.5);
  const Scalar sp = sin(pitch * 0.5);
  const Scalar cy = cos(yaw * 0.5);
  const Scalar sy = sin(yaw * 0.5);
...
Eigen::Matrix<typename Derived::Scalar, 3, 1> QuaternionToEulerAngles(
  ...
  const Scalar sinp = sqrt(1 + 2 * (q.w() * q.y() - q.x() * q.z()));
  const Scalar cosp = sqrt(1 - 2 * (q.w() * q.y() - q.x() * q.z()));
  angles.y() = Scalar(2) * atan2(sinp, cosp) - kPi / 2;
...
#endif  // ATTITUDE_CONTROLLER_ROTATION_HPP_
